...
function ProductsGallery() {
    const [CDs, setCDsProducts] = useState([]);
    ...
    useEffect(() => {
        fetchDataForCategory('CD', 24, 1)
            .then((data) => setCDsProducts(data));
    }, []);

    return (
        <div>
            <hr className="divider" />
            <ProductList title="All CDs" products={CDs} />
            <hr className="divider" />
        </div>
    );
}
